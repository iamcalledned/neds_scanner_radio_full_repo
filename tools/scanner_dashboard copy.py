...
def safe_id(s: str) -> str:
    """
    Textual IDs may contain only letters, numbers, underscores, or hyphens.
    Convert anything sketchy into underscores.
    """
    cleaned = []
    for ch in s:
...
def clean_proc(p: str) -> str:
    """
    Convert nvidia-smi process output into a safe display string.
    Also sanitize bracket characters so Textual markup doesn't try
    to interpret process names as markup.
    """
    parts = p.split()
    ...
    # Prevent Textual markup parsing from choking on stray brackets
    ...
    with open(TRANSCRIBER_ENV_FILE, "r") as f:
        for line in f:
            ...
            if sline.startswith("DEFAULT_MODEL_KEY="):
                ...
                if val:
                    secondary_models = [x.strip() for x in val.split(",") if x.strip()]
...
def update_transcriber_env(
    default_model: str,
    default_enabled: bool,
    secondary_models_str: str,
    ...
    for line in lines:
        ...
        if (
            sline.startswith("DEFAULT_MODEL_KEY=")
            or sline.startswith("#DEFAULT_MODEL_KEY=")
            or sline.startswith("# DEFAULT_MODEL_KEY=")
        ):
        ...
        if (
            sline.startswith("SECONDARY_MODELS=")
            or sline.startswith("#SECONDARY_MODELS=")
            or sline.startswith("# SECONDARY_MODELS=")
        ):
...
class ModelConfigPanel(Vertical):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "save_config_btn":
            ...
            sec_models = [
                str(cb.label)
                for cb in self.query(".sec_cb", Checkbox)
                if cb.value
            ]
