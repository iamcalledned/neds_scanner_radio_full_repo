...
config_file="${TRAINING_CONFIG:-${training_dir}/training_config_v108_hybrid.json}"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to read the hybrid training config." >&2
  exit 1
fi

python="${venv_dir}/bin/python"
base_model="$(jq -er '.model' "${config_file}")"
source_manifests="$(jq -er '.source_manifests_dir' "${config_file}")"
word_manifests="$(jq -er '.word_manifests_dir' "${config_file}")"
negative_manifests="$(jq -er '.negative_manifests_dir' "${config_file}")"
...
run_stage() {
  ...
  local run_test="$5"
  local stage_name epochs learning_rate augment_probability eval_steps save_steps
...
echo "Hybrid config: ${config_file}"

run_stage stage1 "${base_model}" "${stage1_dir}" "${word_manifests}" --no-run-test
run_stage stage2 "${stage1_dir}/best" "${stage2_dir}" "${negative_manifests}" --no-run-test
run_stage stage3 "${stage2_dir}/best" "${stage3_dir}" "${source_manifests}" --run-test

echo
echo "Hybrid V108 training complete."
