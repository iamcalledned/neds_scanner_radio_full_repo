...
This script serves as a long-lived Multi-purpose Control Process (MCP) server that manages a Whisper model loaded on GPU and provides various transcription-related services.
"""
...
def _json_from_env(env_value: str, file_path: str) -> Optional[Any]:
    """Retrieve JSON data from environment variables or file."""
    return json_from_env(env_value, file_path)


def _resolve_model_ref(model_ref: str) -> str:
    """Resolve a model reference to a specific model path."""
...
def _merged_transcribe_settings(raw: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge raw settings with default transcription settings."""
...
def _build_transcribe_kwargs(
    *,
    ...
    """Build transcription keyword arguments based on provided parameters."""
...
def _build_routing_rules() -> list[RoutingRule]:
    """Build a list of routing rules for model selection."""
...
def _build_router() -> ModelRouter:
    """Build a router for managing model selection and caching."""
...
def _ensure_runtime() -> Dict[str, Any]:
    """Ensure the runtime environment is properly set up."""
...
def try_import_db() -> None:
    """Try importing the database module if enabled."""
...
async def _wal_checkpoint_loop():
    """Start a background task to periodically checkpoint the WAL."""
...
def _get_state(ctx: Context) -> WhisperState:
    """Get the current state of the MCP."""
...
def _get_router(ctx: Context) -> Optional[ModelRouter]:
    """Get the current router of the MCP."""
...
def _transcribe_with_state(
    ...
    *,
    ...
    """Transcribe a file with a given state."""
